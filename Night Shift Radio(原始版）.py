...
def _fresh():
    st = {
        "seed": 0x40464046, "calls": 0, "phase": "menu",
...
def _start_call(st, rng, phase="call"):
    st["deck"] = rng.shuffle(_new_deck())
...
def _score(cards, jokers, first_play=False, return_triggered=False):
...
def _table(st, hide=False):
    c = _CALLERS[st["night"]] if st["night"] < len(_CALLERS) else None
    lines = ["┌" + "─" * 46 + "┐"]
    fog = st.get("fog")
    if c:
        if fog:
            lines.append(f"│ 来电 {st['night']+1}/6：{c['name']} ｜ 胃口 {_appetite(c['target'])}")
            lines.append(
                f"│ 信号 {_meter(st['progress'], c['target'])} ｜ 播出 {_dots(st['hands_left'],3)} ｜ "
                f"重抽 {_dots(st['redraws_left'],1)} ｜ 保险丝 {_dots(st['fuse'],2,'█','▒')}"
            )
        else:
            lines.append(f"│ 来电 {st['night']+1}/6：{c['name']} ｜ 目标 {c['target']}")
    ...
    lines.append("│ 手牌：" + _hand_text(st["hand"]))
    if fog:
...
def _after_call(st, win):
    ...
    if win:
        st["cleared"] += 1
        st["night"] += 1
        if st["night"] >= len(_CALLERS):
            st["phase"] = "victory"
            st["pending"] = []
            return "\n天亮了。天线停止冒汗。你活到了下播。"
        if len(st["jokers"]) < 4:
            st["pending"] = _sample(rng, [j for j in _JOKERS if j not in st["jokers"]], 2)
            if st["pending"]:
                _start_call(st, rng, phase="pick")
                st["seed"], st["calls"] = rng.state, rng.calls
                return "\n来电断了。走廊尽头有两张小丑牌在看你。pick 1 / pick 2 / skip"
        st["pending"] = []
        _start_call(st, rng, phase="between")
        st["seed"], st["calls"] = rng.state, rng.calls
        return "\n这档电话被你哄睡了。next 接下一通。"
    st["failed"] += 1
    st["fuse"] -= 1
...
def cmd(text="help"):
    ...
    if c == "status":
        ...
        if st["phase"] == "pick" and st.get("pending"):
            if st.get("fog"):
                ...
                extra = "\n可选小丑：" + "；".join(f"{i+1}.{j}：{_JOKERS[j]['desc']}" for i, j in enumerate(st["pending"]))
    ...
    if c == "play":
        ...
        cur = _CALLERS[st["night"]]
        remaining = cur["target"] - st["progress"]
        signal, chips, mult, tags, trig = _score(cards, st["jokers"], first, return_triggered=True)
